...
//注册文件容器格式。 
//ffplay 把所有支持的文件容器格式用链表串联起来，表头是 first_iformat。
...
int match_ext(const char *filename, const char *extensions)
{
    ...
    if (ext)
    {
        ...
        for (;;)
        {
            //件名中可能有多个标点符号，取两个标点符号间或一个标点和一个结束符间的字符串和扩展名比较
            //来判断文件类型，所以可能要多次比较，所以这里有一个循环
            q = ext1;
...
AVInputFormat *av_probe_input_format(AVProbeData *pd, int is_opened)
{
    ...
    int score, score_max;
    //core， score_max 可以理解识别文件容器格式的正确级别。文件容器格式识别结果，如果完全正确可
    ...
    for (fmt1 = first_iformat; fmt1 != NULL; fmt1 = fmt1->next)
    {
        ...
        {
            //过文件扩展名来识别文件容器格式，因为文件扩展名任何人都可以改，如果改变扩展名，这种方法
            //就错误，如果不改变扩展名，这种识别方法有点可靠，综合等级为 50
            if (match_ext(pd->filename, fmt1->extensions))
                score = 50;
        }
        /*
            如果识别出来的等级大于最大要求的等级，就认为正确识别，相关参数赋值后，进下一个循环，最后
            返回最高级别对应的文件容器格式
        */
        if (score > score_max)
        {
						 ...
						 AVInputFormat *fmt, AVFormatParameters *ap)
{
    ...
    //ic->iformat->read_header(ic, ap)函数中赋值
					   ...
					   int buf_size, AVFormatParameters *ap)
{
    ...
    if (!fmt || must_open_file)
    {
        ...
        //读 PROBE_BUF_MIN(2048)字节文件开始数据识别文件格式，如果不能识别文件格式，就把识别文
        //件缓存以 2 倍的增长扩大再读文件开始数据识别，直到识别出文件格式或者超过 131072 字节缓存 
...
/*
    加索引到索引表。有些媒体文件为便于 seek，有音视频数据帧有索引， ffplay 把这些索引以时间排
    序放到一个数据中。返回值添加项的索引
*/
