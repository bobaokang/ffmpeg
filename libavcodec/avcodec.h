...
typedef struct AVCodecContext
{
    ...
    unsigned char *extradata; // Codec????????????Audio??WAVEFORMATEX?????????
    int extradata_size; // ??Video??BITMAPINFOHEADER?????????
...
typedef struct AVCodec
{
    ...
    enum CodecType type;    //编解码类型 音频/视频/数据
    enum CodecID id;        //标示 Codec 的 ID，有 CODEC_ID_MSRLE， CODEC_ID_TRUESPEECH 等
    int priv_data_size;     // 标示具体的 Codec 对应的 Context 的大小，在本例中是 MsrleContex
